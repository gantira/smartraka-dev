                ...
                <div class="tab-pane fade show active" id="user-list" role="tabpanel">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">Submission of funds List</h3>
                            ...
                            <div class="table-responsive">
                                <table class="table table-sm table-striped table-hover table-vcenter text-nowrap mb-0">
                                    ...
                                    <tbody>
                                        @forelse ($sofs as $sof)
                                            <tr>
                                                <td>{{ $sof->company->name }}</td>
                                                <td class="text-wrap">{{ $sof->description }}</td>
                                                <td>{{ rupiah($sof->price) }}</td>
                                                <td>{!! $sof->status_label !!}</td>
                                                <td>{{ $sof->created_at->format('m/d/Y') }}</td>
                                                <td>
                                                    <button wire:click="$emit('edit', {{ $sof->id }})" type="button"
                                                        class="btn btn-icon btn-sm btn-sm" title="Edit"
                                                        data-toggle="tooltip" data-placement="top"><i
                                                            class="icon-note"></i></button>

                                                    <button wire:click="showConfirmation({{ $sof->id }})" type="button"
